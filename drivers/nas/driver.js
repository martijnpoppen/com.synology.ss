...
module.exports = class NasDriver extends Homey.Driver {
  async onPair(socket) {
    ...
    socket.on('nas-api', (data, callback) => {
  ...
  async validateAPI(socket, data) {
    ...
    const response = await fetch(url, {
      ...
      this.log(error);
      // result not ok
      socket.emit('nas-api-error', error.message, (err, data) => {
        this.log(data);
      });
    });
    ...
    if (response !== undefined && response.data.sid !== undefined) {
      socket.emit('nas-api-ok', response.data.sid, (err, data) => {
        this.log(data);
      });
    } else if (response !== undefined) {
      // result not ok
      socket.emit('nas-api-error', 'API login failed', (err, data) => {
        this.log(data);
      });
    }
  }
