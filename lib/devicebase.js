...
class DeviceBase extends Homey.Device {
  ...
  async storePairData(protocol, host, port, sid, did, account, password, init = false) {
    this.log(protocol, host, port, sid, did, account, password, init);
    ...
    await this.setStoreValue('account', account).catch(this.error);
    await this.setStoreValue('password', password).catch(this.error);

    await this.storeSession(sid, did, init);
  }
  ...
  async getAPIUrl(path = '', qs = {}) {
    ...
    if (stationId !== undefined && stationId !== null) {
      const sd = ManagerDrivers.getDriver('station').getDevice({ id: `${stationId}` });
  ...
  async getNewSession() {
    ...
    const credentials = await device.getDriver().getDecryptedAccount(accountData);
